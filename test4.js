...
//	console.log(fSelect.name);
	...
	//Populate form select with product number as options
	var select = document.createElement("P");
//	select.value = values[2];
	select.innerHTML = values[2];

	//fSelect.appendChild(select);
	console.log(fSelect);
...
function fetchDataForObject(obj) {
    ...
    $.ajax({
	    ...
	    error: function (xhr) {
		// Handle errors here
		    console.error('Failed to fetch data for ' + obj.NUMBER + ': Error was;' + xhr.status )
	    },
...
csvArray.forEach(obj => {
	...
	window.setInterval(fetchDataForObject(obj), 80000);
});
